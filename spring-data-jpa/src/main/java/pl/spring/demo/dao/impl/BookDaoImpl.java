...
public class BookDaoImpl extends AbstractDao<BookEntity, Long> implements
		BookDao {
	...
	@Override
	public List<BookEntity> findBooksBySearchCriteria(
		...
		if (searchCriteria != null) {
			...
			if (!StringUtils.isEmpty(searchCriteria.getAuthor())) {
				...
				predicate.and(bookEntity.authors.any().personalData.name
						.startsWithIgnoreCase(author).or(
								bookEntity.authors.any().personalData.surname
										.startsWithIgnoreCase(author)));
			}
			if (!StringUtils.isEmpty(searchCriteria.getLibraryName())) {
				...
				predicate.and(new JPASubQuery()
						.from(libEntity)
						.where(libEntity.books.any().bookEntity.title
								.eq(bookEntity.title)).exists());
			}
